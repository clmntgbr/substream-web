...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const response = await fetch(`${BACKEND_API_URL}/streams/${id}/download/subtitle`, {
      method: "GET",
      headers: {
    ...
    const downloadResponse = new NextResponse(blob, {
      ...
      headers: {
        ...
        "Content-Disposition": contentDisposition || `attachment; filename="subtitle-${id}.srt"`,
      },
    });
    ...
    if (error instanceof Error) {
      ...
      return NextResponse.json({ error: `Failed to download subtitle: ${error.message}` }, { status: 500 });
    }
