...
export function VideoQueueCard({ stream }: VideoQueueCardProps) {
  ...
  const handleDownload = () => {
    if (!stream.isDownloadable || !stream.id) {
      toast.error(translations.home.queue.actions.streamNotAvailable);
      return;
    }
  ...
  const handleDownloadSubtitle = () => {
    if (!stream.isSrtDownloadable || !stream.id) {
      toast.error(translations.home.queue.actions.subtitleNotAvailable);
      return;
    }
  ...
  const handleDownloadResume = () => {
    if (!stream.isResumeDownloadable || !stream.id) {
      toast.error(translations.home.queue.actions.resumeNotAvailable);
      return;
    }
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="ghost"
              size="icon"
              className="data-[state=open]:bg-muted size-8 cursor-pointer absolute top-0 right-0 rounded-l-full rounded-br-full bg-accent/50"
            >
              <MoreVertical />
