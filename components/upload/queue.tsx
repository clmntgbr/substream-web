      ...
      <div className="container mx-auto bg-card border border-border rounded-lg">
          ...
          <div className="flex items-center gap-6 text-sm">
            <div className="flex items-center gap-2">
              ...
              <span className="text-muted-foreground">
                Completed:{" "}
                {state.streams?.filter((stream) => stream.isCompleted).length}
              </span>
            </div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-muted-foreground">
                Processing:{" "}
                {state.streams?.filter((stream) => stream.isProcessing).length}
              </span>
            </div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-muted-foreground">
                Errors:{" "}
                {state.streams?.filter((stream) => stream.isFailed).length}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="">
          {state.streams?.map((file) => (
            <div
              key={file?.id}
              className="p-4 border-b border-border last:border-b-0 hover:bg-muted/30 transition-colors duration-200"
            >
