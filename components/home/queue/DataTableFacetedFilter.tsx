                  ...
                  options
                    ...
                    .map((option) => (
                      <Badge variant="secondary" key={option.value} className="rounded-sm px-1 font-normal">
                        {t.stream.status[option.value as keyof typeof t.stream.status]?.title || option.value}
                      </Badge>
                    ))
                )}
              </div>
              ...
              {options.map((option) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span>{t.stream.status[option.value as keyof typeof t.stream.status]?.title || option.value}</span>
                  </CommandItem>
                );
              })}
            </CommandGroup>
